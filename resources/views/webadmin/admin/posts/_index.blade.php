...
<script type="text/javascript">
    ...
    var table = jQuery('._table').DataTable({
        ...
        buttons : [
            {
                ...
                action  : function()
                {
                    ...
                    jQuery.get('{{ route("posts.create") }}', function(res) {
            ...
            },
            {
                ...
                action  : function(e, dt, node, config)
                {
                    ...
                    if(res != undefined ) {
                        var _url = replaceUrlId('{{ route("posts.edit","ID") }}',res.id);
            ...
            },
            {
                ...
                action  : function(e, dt, node, config)
                {
                    ...
                    jQuery('.modal-body').html(errorAlert("Silakan pilih data terlebih dahulu."));
                    if(res != undefined ) {
                        var _url = replaceUrlId('{{ route("posts.show","ID") }}',res.id);
        ...
        columns : [
            ...
            { data : 'name', name : 'name' },
            { data : 'category[0].name', name : 'category', className : 'text-center' },
            { data : 'author.name', name : 'author', className : 'text-center' }
        ],
        ajax : { url : "{{ Route('posts.index') }}" }
    });
    /*
    *
    *   < =========== BUTTON ADD =========== >
